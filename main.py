...
def draw_score():
    ...
    if mode == 2:
        mode_text = font.render(
            f'Time Remainning: {time_remaining}', True, 'black')
...
def draw_menu():
    ...
    global time_remaining, best_timed, best_ammo, best_freeplay, write_values, clicked, new_coords
    game_over = False
    pause = False
    screen.blit(menu_img, (0, 0))
    mouse_pos = pygame.mouse.get_pos()
    clicks = pygame.mouse.get_pressed()
    free_play_button = pygame.rect.Rect((170, 524), (260, 100))
    ...
    if free_play_button.collidepoint(mouse_pos) and clicks[0] and not clicked:
        mode = 0
        level = 1
        menu = False
        time_passed = 0
